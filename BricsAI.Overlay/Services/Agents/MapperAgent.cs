...
namespace BricsAI.Overlay.Services.Agents
{
    public class MapperAgent : BaseAgent
    {
        ...
        public async Task<(string ActionPlan, int Tokens, int InputTokens, int OutputTokens)> DeduceLayerMappingAsync(string layerName, string geometricFootprint)
        {
...
";
            ...
            var result = await CallOpenAIAsync(systemPrompt, prompt, expectJson: true);
